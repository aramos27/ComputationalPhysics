...
function rk4_step(r::Vector{Float64}, v::Vector{Float64})
    ...
    k4r = τ * (v + k3v)
    k4v = τ * compute_acceleration(r + k3r, v + k3v)
...
function classify_endpoint(r::Vector{Float64})
    ...
    if min_dist >0.05
        println("Error in classification")
    end
...
function grid_simulation_multithread(gridLength::Int)
    ...
    Threads.@threads for i in 1:gridLength
        x0 = grid[i]
        ...
        for j in 1:gridLength
            ...
            if done % 1000 == 0
                println("Progress: $((done/total) * 100) %")
            end
        end
...
function plot_classification_grid(classification::Array{Int,2})
    ...
    value_to_color = Dict(
    1 => parse(Colorant, "#FF6F00"),  
    2 => parse(Colorant, "#3B9C9C"),  
    3 => parse(Colorant, "#8A2BE2")   
)
...
function simulate_and_plot(initial_r::Vector{Float64}, initial_v::Vector{Float64})
    ...
    for i in 1:max_steps
        ...
        if norm(v_new) < 1e-4 && norm(r_new - r) < 1e-4
...
function main()
    ...
    elapsed = time() - start
    println("Simulation took $(elapsed) seconds")
    ...
    initial_r = [-1.3, 1.3]
    initial_v = [0.0,0.0]
    fig1 = simulate_and_plot(initial_r, initial_v)
    save("AnimationPlot0.5.png", fig1)
